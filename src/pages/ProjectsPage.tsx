...
import { StatusBadge, getStatusDetails } from '../components/common/StatusBadge';
...
export const ProjectsPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="glass-card rounded-[24px] p-4 flex flex-col md:flex-row items-center justify-between gap-4">
        {/* Search */}
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 rounded-xl border border-slate-200/60 dark:border-white/10 bg-slate-100/80 dark:bg-slate-800/60 text-xs text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#45cc42]"
          />
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto flex-wrap justify-between md:justify-end">
          ...
          <div className="flex items-center gap-1.5 text-xs text-slate-500 font-bold">
            ...
            <select
              ...
              className="px-3 py-1.5 rounded-xl border border-slate-200/60 dark:border-white/10 bg-slate-100/80 dark:bg-slate-800/60 text-xs font-bold text-slate-800 dark:text-slate-200 outline-none"
            >
          ...
          <div className="flex items-center gap-1 bg-slate-100/80 dark:bg-slate-800/60 p-1 rounded-xl border border-slate-200/60 dark:border-white/10">
            <button
              ...
              className={`p-1.5 rounded-lg transition ${
                viewStyle === 'cards'
                  ? 'bg-white dark:bg-slate-900 text-[#052b66] dark:text-[#45cc42] shadow-2xs font-bold'
            ...
            {filtered.map((project, index) => {
              ...
              return (
                <motion.div
                  ...
                  whileHover={{ y: -4 }}
                  className="glass-card rounded-[28px] p-6 shadow-xs card-hover flex flex-col justify-between space-y-4"
                >
                  <div className="space-y-3">
                    ...
                    {/* Progress Indicator for active pipeline stages */}
                    {project.status !== 'completed' && project.status !== 'failed' && (
                      <div className="p-3 bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/80 rounded-xl space-y-2">
                        <div className="flex items-center justify-between text-[11px] font-semibold text-slate-700 dark:text-slate-300">
                          <span className="capitalize text-[#052b66] dark:text-[#45cc42] font-bold">
                            Stage: {project.status}...
                          </span>
                          <span className="font-mono">{statusDetails.percentage}%</span>
                        </div>
                        <div className="w-full h-2 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden">
                          <motion.div
                            ...
                            className={`h-full ${statusDetails.progressClass}`}
                          />
                        </div>
                      </div>
                    )}
